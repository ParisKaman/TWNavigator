...
#pragma warning restore CS1998
        {
            ...
            session.Configuration.AccountId = "mixedreality.azure.com";
            ...
            Debug.Log("Session before : " + session);
            Debug.Log("Session.Session before : " + session.Session);
            Debug.Log("arSession before : " + arSession);
            DebugDeeper(arSession.transform);
            Debug.Log("arSession.subsystem.nativePtr before : " + arSession.subsystem.nativePtr);
            Debug.Log("arSession.subsystem.nativePtr.GPP() before : " + arSession.subsystem.nativePtr.GetPlatformPointer());
            session.Session = arSession.subsystem.nativePtr.GetPlatformPointer();
            Debug.Log("Session after : " + session);
            Debug.Log("Session.Session after : " + session.Session);
            Debug.Log("arSession after : " + arSession);
            Debug.Log("arSession.subsystem.nativePtr after : " + arSession.subsystem.nativePtr);
            Debug.Log("arSession.subsystem.nativePtr.GPP() after : " + arSession.subsystem.nativePtr.GetPlatformPointer());
#elif UNITY_WSA || WINDOWS_UWP
            // No need to set a native session pointer for HoloLens.
#else
            throw new NotSupportedException("The platform is not supported.");
#endif
            Debug.Log("CreateSessionAsync:");
            Debug.Log("session: " + session);
            Debug.Log("Session.Session: " + session.Session);
        ...
        void DebugDeeper(Transform trans)
        {
            ...
            var list = trans.GetComponents<MonoBehaviour>();
            for (int i = 0; i < list.Length; i++)
            {
                Debug.Log(trans.name + " Component " + i + ": " + list[i].name);
            }
